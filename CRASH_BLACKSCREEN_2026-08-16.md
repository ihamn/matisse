...
—— matisse 现场
# 归因修正：卡死也污染 dsh（我之前漏了对比，用户纠正）(2026-08-16)

> 修正 CRASH_BLACKSCREEN 的补充证据。我此前结论"卡死不污染"是错的——
> 只看了第二次卡死，漏了第一次。

## 修正后的时间线（隔离目录 mtime 为铁证）
| 时间 | 事件 | dsh |
|------|------|-----|
| 15:28 | crash#3 (panic) | ❌ 污染 (storages.corrupt-20260816) |
| **20:37** | **第一次卡死 (boot 25db1657, A1 round2)** | ❌ **污染 (storages.corrupt-20260816b)** |
| 21:07 | 第二次卡死 (boot b55f4db5) 后 | ✅ 干净 (当前 session_projcache 21:07 正常) |

## 修正结论
1. **卡死也污染 dsh**（第一次卡死 20:37 产生了 20260816b）—
   "卡死 vs panic 区分"不成立
2. 但第二次卡死没污染 → **污染不是必然的**
3. 更可能: 污染与**重启时文件系统是否干净关盘**有关（随机/状态相关），
   不是"事故类型"决定
4. 我此前"panic 污染 / 卡死干净"的二分是错的, 已纠正

## 教训
- 对比必须做全（用户指出的）: 只看最后一次 = 幸存者偏差
- 隔离目录 mtime 是可靠证据（20:37:59 = 第一次卡死时刻, 精确吻合）

—— matisse 现场 (2026-08-16 21:10)
