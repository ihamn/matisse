"""Extract boot.img with proper parsing and find kernel"""
...
for p_raw in manifest.get(13, []):
    ...
    # First pass: determine total size by finding max extent
    max_offset = 0
    op_info = []
    for op_idx, op_raw in enumerate(operations):
        ...
        for start_block, num_blocks in dst_extents:
            ...
            if end_block > max_offset:
                max_offset = end_block
        
        op_info.append((op_type, dst_extents, op_dict))
    
    total_size = max_offset * BLOCK_SIZE
    print(f"Boot partition total size: {total_size} bytes ({max_offset} blocks)")
    
    # Create output buffer
    boot_img = bytearray(total_size)
    
    # Second pass: apply operations
    for op_idx, (op_type, dst_extents, op_dict) in enumerate(op_info):
        if op_type == 0:  # REPLACE
            ...
            for start_block, num_blocks in dst_extents:
                ...
                chunk = chunk[dst_len:]
            print(f"  Op {op_idx}: REPLACE {data_length} bytes at {[(s*BLOCK_SIZE, n*BLOCK_SIZE) for s,n in dst_extents]}")
        ...
        elif op_type in (1, 8):  # REPLACE_BZ, BROTLI_BSDIFF
            for start_block, num_blocks in dst_extents:
                dst_offset = start_block * BLOCK_SIZE
                dst_len = num_blocks * BLOCK_SIZE
                print(f"  Op {op_idx}: NEED_SOURCE type={op_type} at offset {dst_offset} len {dst_len}")
    
    # Save full boot image
    ...
    if pos >= 0:
        ...
        print("\nNo ANDROID! header found - boot image is incomplete")
        # Try to find kernel in the raw data
        for magic_name, magic_bytes in [
            ("gzip", b"\x1f\x8b"),
            ("XZ", b"\xfd\x37\x7a\x58"),
        ]:
            for m_idx in range(min(10, boot_img.count(magic_bytes))):
                pos = boot_img.find(magic_bytes)
                if pos >= 0:
                    print(f"Found {magic_name} at offset {pos}")
                    try:
                        if magic_name == "gzip":
                            decomp = gzip.decompress(boot_img[pos:pos+50*1024*1024])
                        else:
                            decomp = lzma.decompress(boot_img[pos:pos+50*1024*1024])
                        if b"Linux version" in decomp:
                            idx = decomp.find(b"Linux version")
                            end = decomp.find(b"\n", idx)
                            if end == -1: end = idx + 200
