...
rsh(){ local cmd="$1" t="${2:-60}"
  if [ "$RISH_MODE" = args ]; then (cd "$RISH_DIR" && timeout "$t" ./rish "$cmd") 2>&1
  else printf '%s\n' "$cmd" | (cd "$RISH_DIR" && timeout "$t" ./rish) 2>&1; fi; }
rpush(){ local l="$1" r="$2"
  if [ "$RISH_MODE" = args ]; then (cd "$RISH_DIR" && timeout 180 ./rish "cat > '$r'") < "$l" 2>&1
  else { echo "cat > '$r'"; cat "$l"; } | (cd "$RISH_DIR" && timeout 180 ./rish) 2>&1; fi; }
...
if [ -n "$SHA_EXP" ] && [ "$SHA_GOT" != "$SHA_EXP" ]; then say "!! SHA 不一致,中止开火(防错二进制)"; exit 3; fi
say "二进制校验通过"
...
while :; do
  ...
  say "settle=$SETTLE load=$LOAD (第${LOADWAIT_N}次等待)"
  [ "$LOAD" = "LOAD_GO" ] && break
  LOADWAIT_N=$((LOADWAIT_N+1)); [ "$LOADWAIT_N" -ge 3 ] && { say "裁定: 照打(评审附录A)"; break; }
  say "负载高,等10分钟再试..."; sleep 600
done
...
BOOT1=$(rsh "cat /proc/sys/kernel/random/boot_id" 20 | tr -d '\r')
if [ "$R7_GATE" = "R_LANDED" ] && [ "$BOOT1" = "$BOOT0" ]; then
  ...
  BOOT2=$(rsh "cat /proc/sys/kernel/random/boot_id" 20 | tr -d '\r')
...
git add -A
git commit -q -m "termux auto card4 ${RUN_TS}: R7=${R7_GATE} R8=${R8_GATE:-skip} canary-off armed-write (auto field script)" || true
for i in 1 2 3; do
